...
Проект использует продвинутую систему логирования с автоматической ротацией по датам:

- **Файлы логов**: сохраняются в папку `logs/` с именем `bot-YYYY-MM-DD.log`
- **Автоматическая ротация**: каждый день создается новый файл
- **Автоматическая очистка**: старые логи удаляются через 30 дней
- **Консольный вывод**: логи также выводятся в консоль
...
# Следование за логом в реальном времени
